...
def build_graph(classifier=None, retriever=None, approver=None,
    ...
    def advise_node(state):
        # Second tool: consult the LLM for a human-only policy hint. Purely
        # advisory - it neither reads into nor changes y_pred, severity, findings
        # or the verdict. It only adds an optional note for the reviewer. Gated
        # by the same band as the verdict, so it uses the LLM exactly when the
        # rest of the assessment does (and always when llm_band=None).
        proba = state["proba"]
        in_band = llm_band is None or (llm_band[0] <= proba <= llm_band[1])
        advice = policy_advice(state["proposal_text"],
                               state.get("matched_policies", []),
                               use_llm=in_band)
        return {"advisory_note": advice["note"], "advisory": advice}
    ...
    # assess -> advise (second tool) -> route on the assessment's own verdict.
    graph.add_edge("assess", "advise")
    graph.add_conditional_edges("advise", route_after_assess,
