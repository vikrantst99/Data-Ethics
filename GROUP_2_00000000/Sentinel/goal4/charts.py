...
def fig_trigger_words(trigger_words, n=10, ink=None):
    ...
    ax.legend(handles=[Patch(color=RED, label="toward Red Flag"),
              ...
              fontsize=8, frameon=False, labelcolor=ink["text"])
    _style(ax, ink)
    return fig
...
def fig_policy_scores(retrieved, matched_ids, severities, ink=None):
    ...
    for label in ax.get_xticklabels():
        ...
        label.set_ha("right")
    # Legend below the rotated tick labels so it never covers the bars.
    ax.legend(handles=handles, loc="upper center", bbox_to_anchor=(0.5, -0.24),
              ncol=4, fontsize=8, frameon=False, labelcolor=ink["text"])
    _style(ax, ink)
    return fig
...
def fig_probability(proba, threshold=0.40, ink=None):
    ...
    fig, ax = plt.subplots(figsize=(5.8, 1.5), layout="constrained")
    ...
    ax.set_title("Red-Flag probability", fontsize=11, loc="left", pad=10)
    _style(ax, ink)
    return fig
